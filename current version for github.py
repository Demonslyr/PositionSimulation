...
token = {
  ...
  'Authorization': "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..eAfy2kPYVne7Wj_keYgFVArt8MdKRe2bch_NdK7fRpz5kO0lEd6NBwrqKKvfJniY-4c-l77brFa8glSLxJhFRPtDTA7YJUxQ4gbDfELTbzO9rAhtrlknNT_eC381uJBAp6t8v-HV2sf-FcVLFYUDrs2dfsEiHm060KKZYQxTOVOxJ84F4aFOKi52ya9QY6BsaP5pPQpdXNw1r6TB5p7Q4nKGbSXPvxuBFCalTqlNlVlgRYQh6MOdgykQmUYGPiWLdYa9iU4MpaTU6xTgb9rsdWczbk9T_omX00cL4shv7RZe9R72fNU7ae2vVurgl-Hkp3EMz8zchJE8EFPdHKVZXQ-Zax2r7hP7QDQsl-Z17FI4YWFqa9sisk6JuGolGb1B4Ry88U00cP4Yg9Yn5ACjXdsiv82tn2Hu9aCTfcd-SS34mh5M7uv2rYMBr1QOHiIdBu87CFdnOOLk1i3yhyPBEo-wlYBY7UTTS8XvAf8F-Ee4qMe2g7qJ8kJPLYKdO9sImJM-_hMMX7lQ-YXt3613NVUm3FDEH8JRX2TNehaT7BLg3_DPbc2kQMvP5Zap4qchV6NZXHFHlOAe_JzcrXMr_7qvFUic_yxEJL3_JzsqiBN4CyKZl6Z3eWeZEeP22SGS5Otysi8KFv2FIDURCxig_hRGf6Bf7GHXkSK6grp4Cks"}
...
def TXY_to_TM(TXY,
              interval=.2500):  # formats the Timestamps and the coordinates into a usable format of [time,total distance moved during that interval]
      ...
      if TM[i][1] > 0:  # was there any movement in this interval?
        ...
        if TM[i][0] - mEndFinder >= Twindow:  # has there been no movement for 3 seconds?
          if len(moves) == 0:  # if it's the first move, just slap that shit into moves
            ...
            0] != mStart:  # making sure that the line hasn't been added yet (for some reason it likes to add multiples of the same movement if I dont add this check)
...
def get_boss_IDs(report):
  ...
  payload = str(
    ...
    report) + str("\\\"){ masterData{actors{name id gameID subType} \\n       }}}\\n\\n  \\n   \\n    }\\n    \\n\"}")
  headers = token
  conn.request("POST", "/api/v2", payload, headers)
  res = conn.getresponse()
  data = res.read().decode("utf-8")
